...
def BastShoe(command):
    ...
    if command.split(' ')[0] == '1' and len(command.split(' ')) > 2:  # here in this block user input is handled and operations are performed
        ...
        current_state.insert(current_position + 1, current_string)
        ...
        lastUndo = True
        return current_string
    elif command.split(' ')[0] == '5' and possible_redo <= 0:  # operation 5, redo. undo_done <=0, so nothing we need to do except returning current string,
        return current_string
    elif command.split(' ')[0] == '5' and possible_redo > 0 and current_position < len(current_state) - 1:  # operation 5, redo and position is not last
        current_position = current_position + 1
        current_string = current_state[current_position]
        undo_done = undo_done - 1
        undo_chain = undo_chain + 1
        possible_redo = possible_redo - 1
        return current_string
    elif command.split(' ')[0] == '5' and possible_redo > 0 and current_position == len(current_state) - 1:  # operation 5, redo and position is last
        undo_chain = undo_chain + 1
        undo_done = undo_done - 1
        possible_redo = possible_redo - 1
        return current_string
