...
def WordSearch(length, s, subs):
    ...
    for i in range(len(output_list)):  # теперь пройдем по списку из нарезанных слов и расположим помещающиеся слова в общие строки
        ...
        if len(output_list[i]) > free_cells_counter and i != len(output_list) - 1:  # слово не помещается и оно не последнее в списке - переходим на следующую строку
        ...
        elif len(output_list[i]) > free_cells_counter and i == len(output_list) - 1:  # слово не помещается и оно последнее в списке - переходим на следующую строку
            output_list2.append(current_working_string)  # запишем в результат то, что уже находится в текущей строке
            current_working_string = ''  # обнуляем текущую рабочую строку, так как предполагается переход на следующую строку
            current_working_string = current_working_string + output_list[i]  # запишем текущее слово и пробел в рабочую строку
            output_list2.append(current_working_string)

        elif len(output_list[i]) < free_cells_counter and words_in_string == 0 and i != len(output_list) - 1: # первое слово в строке, не последнее в списке по длине меньше строки и строка влезает
        ...
        elif len(output_list[i]) < free_cells_counter and words_in_string == 0 and i == len(output_list) - 1:  # первое слово в строке, последнее в списке по длине меньше строки и строка влезает
            current_working_string = ''
            current_working_string = output_list[i]  # тогда запишем текущее слово в рабочую строку
            output_list2.append(current_working_string)  # добавим в результирующий список текущую строку
 
        elif len(output_list[i]) == free_cells_counter and words_in_string == 0 and i != len(output_list) - 1:  # первое слово в строке, не последнее в списке и строка влезает впритык
            current_working_string = output_list[i]  # тогда запишем текущее слово в рабочую строку
            output_list2.append(current_working_string)  # добавим в результирующий список текущую строку
            free_cells_counter = length  # уменьшим счетчик слов на длину записанного слова и еще один пробел после него
            words_in_string = 0

        elif len(output_list[i]) == free_cells_counter and words_in_string == 0 and i == len(output_list) - 1:  # первое слово в строке, последнее в списке и строка влезает впритык
            current_working_string = output_list[i]  # тогда запишем текущее слово в рабочую строку
            output_list2.append(current_working_string)  # добавим в результирующий список текущую строку

        elif len(output_list[i]) <= free_cells_counter and words_in_string > 0 and i != len(output_list) - 1:   # слово влезает, но оно в строке не первое, не последнее в списке - добавляем пробелы в условие
        ...
        elif len(output_list[i]) <= free_cells_counter and words_in_string > 0 and i == len(output_list) - 1:   # слово влезает, но оно в строке не первое, последнее в списке - добавляем пробелы в условие
            current_working_string = current_working_string + output_list[i]
            output_list2.append(current_working_string)
